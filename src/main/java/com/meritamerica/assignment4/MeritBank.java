...
public static boolean processTransaction(Transaction transaction) throws NegativeAmountException, ExceedsAvailableBalanceException, ExceedsFraudSuspicionLimitException {
	...
	try {
		...
		throw e;
	}catch (ExceedsAvailableBalanceException f) {
		throw f;
	}catch (ExceedsFraudSuspicionLimitException g) {
		throw g;
	}
return true;
}

public static FraudQueue getFraudQueue1() {
	return null;
}
public static BankAccount getBankAccount1(long accountId) {
	if(accountId == 0) {
	}
	return null;
}
